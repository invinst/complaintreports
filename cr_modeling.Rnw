...
more complaints. That's about 1 in 1000. (Figure~\ref{po-complaints-fig})
...
for (i in 1:6) {
    ...
    avg = units[i, 'avg']
    n = units[i, 'n']
    hist(officer_units[officer_units$current_unit == unit, 'count'],
         main = unit_names[as.character(unit)],
         xlab="Number of complaints",
         ylab="Police Officers")
}    
