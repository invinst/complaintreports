...
<<punishments>>=
excessive_force <- dbGetQuery(con,
...
suspension <- ifelse((substr(excessive_force$final_discipline, 1, 1) == 0
                      & excessive_force$final_discipline != '000'),
                     TRUE,
                     FALSE)
...
suspension_length <- ifelse(substr(excessive_force$final_discipline, 1, 1) == '0',
                            as.numeric(as.character(excessive_force$final_discipline)),
                            suspension_lookup[as.character(excessive_force$final_discipline)])


less_than_five <- sum(suspension_length <= 5)/length(suspension_length)
...
complaints_by_ipra <- dbGetQuery(con, paste("select complaint, ipra from accused inner join (select complaint_id, investigator, case when (investigator in ('", paste(iad_investigators, collapse = "', '"), "') OR investigator in (SELECT officer_name from officers where current_unit = '121') OR investigator IN (SELECT name FROM police_employees WHERE position = 'POLICE AGENT')) then 'IAD' when name is null then 'CPD' when name is not null then 'IPRA' end as ipra from complaints left join ipra_employees on complaints.investigator = ipra_employees.name) as investigators using (complaint_id)", sep=''))

complaints <- dbGetQuery(con, paste("select investigator from complaints where investigator not in ('", paste(iad_investigators, collapse = "', '"), "') AND investigator not in (SELECT officer_name FROM officers) AND investigator NOT IN (SELECT name FROM police_employees WHERE position = 'POLICE AGENT') AND investigator not in (SELECT name from ipra_employees)", sep=''))
