...
relations <- dbGetQuery(con,
                "select a.officer_id, b.officer_id, count(distinct complaint_id) from accused as a inner join accused as b using (complaint_id) where a.officer_id < b.officer_id group by a.officer_id, b.officer_id having count(distinct complaint_id) > 2")

g <- graph.data.frame(relations, directed=FALSE)

num_complaints <- dbGetQuery(con,
                             "SELECT officer_id, count(distinct complaint_id) \
                              FROM accused GROUP BY officer_id")
...
g <- delete.vertices(g, 
                     which(attr(V(g), 'names') 
                           %in% 
                           names(num_complaints)[num_complaints <= 5]))
...
plot(g, vertex.size=num_complaints/5, vertex.label='', layout=layout.fruchterman.reingold(g,weights=E(g)))
