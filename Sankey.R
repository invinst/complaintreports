...
inputLabel = paste(labels[1],": ",inputs[1]," ", sep="")
...
# First input label
inputLabel = paste(inputs[1] , '\nComplaints')
fontsize = max(0.5,frInputs[1]*1.5)
...
if(length(inputs)>1){
 ...
 # Draw label
  ...
  fontsize = max(0.5,frInputs[j]*1.5)
...
for(i in 1:(length(losses)-1)){
 ...
 # Draw label
  ...
  fontsize = max(0.5,frLosses[i]*1.5)
...
# Last loss label
lossLabel = paste(labels[length(labels)],": ",losses[length(losses)]," ",unit," (",round(100*frLosses[length(losses)],digits=1),"%)",sep="")
fontsize = max(0.5,frLosses[length(losses)]*1.5)
